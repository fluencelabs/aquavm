...
fn length_functor_for_empty_stream() {
    ...
    let expected_trace = vec![
        executed_state::canon(
            json!({"tetraplet": {"function_name": "", "lambda": "", "peer_pk": init_peer_id, "service_id": ""},
                "values": []} ),
        ),
...
fn length_functor_for_canon_stream() {
    ...
    let expected_trace = vec![
        ...
        executed_state::canon(
            json!({"tetraplet": {"function_name": "", "lambda": "", "peer_pk": init_peer_id, "service_id": ""},
                "values": [{"result": 1, "tetraplet": {"function_name": "", "lambda": "", "peer_pk": init_peer_id, "service_id": ""}, "trace_pos": 0},
                           {"result": 1, "tetraplet": {"function_name": "", "lambda": "", "peer_pk": init_peer_id, "service_id": ""}, "trace_pos": 1}
                ]} ),
        ),
        unused!(2, peer = init_peer_id, service = "..0", args = vec![2]),
    ];
...
fn length_functor_for_empty_canon_stream() {
    ...
    let expected_trace = vec![
        executed_state::canon(
            json!({"tetraplet": {"function_name": "", "lambda": "", "peer_pk": init_peer_id, "service_id": ""}, "values": []} ),
        ),
        unused!(0, peer = init_peer_id, service = "..0", args = vec![0]),
    ];
    assert_eq!(actual_trace, expected_trace);
