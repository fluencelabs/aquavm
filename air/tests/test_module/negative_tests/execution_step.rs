...
use air::{CatchableError, LambdaError};
...
fn value_not_contain_such_array_idx_call_arg_lambda() {
    ...
    let result = peer_vm_1.call(script.clone(), "", "", <_>::default()).unwrap();
    let expected_error =
        air::CatchableError::LambdaApplierError(air::LambdaError::ValueNotContainSuchArrayIdx { value: arg, idx: idx });
    assert!(check_error(&result, expected_error));
}
...
#[test]
fn scalara_accessor_has_invalid_type_ap() {
    let vm_peer_id_1 = "vm_peer_id_1";
