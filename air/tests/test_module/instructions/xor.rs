...
fn xor() {
    ...
    assert_eq!(
        actual_trace[(0 as PosType).into()],
        executed_state::service_failed(1, "failed result from fallible_call_service")
    );
    assert_eq!(actual_trace[(1 as PosType).into()], expected_call_result);
    ...
    assert_eq!(actual_trace[(0 as PosType).into()], expected_call_result);
}
...
fn xor_var_not_found() {
    ...
    let actual_trace = trace_from_result(&result);
    assert_eq!(actual_trace[(0 as PosType).into()], executed_state::par(1, 0));
    assert_eq!(actual_trace[(1 as PosType).into()], executed_state::request_sent_by(local_peer_id));
}
