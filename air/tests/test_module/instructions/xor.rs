...
fn xor() {
    ...
    assert_eq!(
        actual_trace.as_ref()[0],
        executed_state::service_failed(1, "failed result from fallible_call_service")
    );
    assert_eq!(actual_trace.as_ref()[1], expected_call_result);
    ...
    assert_eq!(actual_trace.as_ref()[0], expected_call_result);
}
...
fn xor_var_not_found() {
    ...
    let actual_trace = trace_from_result(&result);
    assert_eq!(actual_trace.as_ref()[0], executed_state::par(1, 0));
    assert_eq!(actual_trace.as_ref()[1], executed_state::request_sent_by(local_peer_id));
}
...
fn last_error_with_xor() {
    ...
    assert_eq!(actual_trace.as_ref()[1], expected_state);
}
