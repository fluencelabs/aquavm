...
pub(super) fn handle_prev_state<'i>(
    ...
    match met_result.result {
        ...
        Failed(ref failed_cid) => {
            ...
            exec_ctx.record_call_cid(&tetraplet.peer_pk, failed_cid.clone());
        ...
        Executed(value) => {
            ...
            match &resulted_value {
                ValueRef::Scalar(ref cid) | ValueRef::Stream { ref cid, .. } => {
                    exec_ctx.record_call_cid(&tetraplet.peer_pk, cid.clone());
                }
                ValueRef::Unused(_) => {}
            }
...
fn handle_service_error(
    ...
    exec_ctx.record_call_cid(&peer_id, service_result_agg_cid.clone());
