...
#[test]
fn parse_peer_id_from_canon_stream_map_index() {
    let source_code = r#"
        (call #%canon["peer_id"] ("service_id" "function_name") [arg])
        "#;

    let actual = parse(source_code);
    let canon_stream_map = CanonStreamMap::new("#%canon", 15.into());
    let index = StreamMapKeyClause::Literal("peer_id");
    let expected = call(
        ResolvableToPeerIdVariable::CanonStreamMapIndex(CanonStreamMapIndex::new(
            canon_stream_map,
            index,
        )),
        ResolvableToStringVariable::Literal("service_id"),
...
#[test]
fn parse_service_id_from_canon_stream_map_index() {
    let source_code = r#"
        (call peer (#%canon["service_id"] "function_name") [arg])
        "#;

    let actual = parse(source_code);

    let canon_stream_map = CanonStreamMap::new("#%canon", 21.into());
    let index = StreamMapKeyClause::Literal("service_id");
    let expected = call(
        ResolvableToPeerIdVariable::Scalar(Scalar::new("peer", 15.into())),
        ResolvableToStringVariable::CanonStreamMapIndex(CanonStreamMapIndex::new(
            canon_stream_map,
            index,
        )),
        ResolvableToStringVariable::Literal("function_name"),
...
#[test]
fn parse_function_from_canon_stream_map_index() {
    let source_code = r#"
        (call peer ("service_id" #%canon["function_name"]) [arg])
        "#;

    let actual = parse(source_code);

    let canon_stream_map = CanonStreamMap::new("#%canon", 34.into());
