...
impl Stream {
    ...
    pub fn iter(&self, generation: Generation) -> Option<StreamIter<'_>> {
        let iter: Box<dyn Iterator<Item = &ValueAggregate>> = match generation {
            ...
            Generation::Nth(generation) => Box::new(
                self.0
                    .iter()
                    .take(generation as usize + 1)
                    .flat_map(|v| v.iter()),
            ),
    ...
    pub fn slice_iter(&self, start: Generation, end: Generation) -> Option<StreamSliceIter<'_>> {
        ...
        let iter: Box<dyn Iterator<Item = &[ValueAggregate]>> = Box::new(
            self.0
                .iter()
                .skip(start as usize)
                .take(len)
...
mod test {
    ...
    fn test_slice_iter() {
        ...
        let slice = stream
            .slice_iter(Generation::Nth(0), Generation::Nth(1))
            .unwrap();
        assert_eq!(slice.len, 2);

        let slice = stream
            .slice_iter(Generation::Nth(0), Generation::Last)
            .unwrap();
        assert_eq!(slice.len, 2);

        let slice = stream
            .slice_iter(Generation::Nth(0), Generation::Nth(0))
            .unwrap();
        assert_eq!(slice.len, 1);

        let slice = stream
            .slice_iter(Generation::Last, Generation::Last)
            .unwrap();
        assert_eq!(slice.len, 1);
    }
