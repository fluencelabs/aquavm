...
use air_values::Generation;
use air_values::Stream;
...
impl Streams {
    ...
    pub(crate) fn meet_scope_end(&mut self, name: String, position: u32) {
        ...
        self.collect_stream_generation(
            name,
            position,
            last_descriptor.stream.generations_count() as u32,
        );
    }
    ...
    fn stream_generation_from_data(
        ...
        self.data_restr_stream_generations
            ...
            .and_then(|scopes| {
                scopes
                    .get(&position)
                    .and_then(|iterations| iterations.get(iteration))
            })
            .copied()
    }
...
impl fmt::Display for StreamDescriptor {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            " <{}> - <{}>: {}",
            self.span.left, self.span.right, self.stream
        )
    }
}
