...
use air_values::FoldIterableState;
use air_values::ScalarRef;
use air_values::ValueAggregate;
...
impl<'i> Scalars<'i> {
        ...
        match self.values.entry(name.into()) {
            ...
            Occupied(entry) => {
                if !shadowing_allowed {
                    return Err(
                        UncatchableError::MultipleVariablesFound(entry.key().clone()).into(),
                    );
                }
    ...
    pub(crate) fn set_iterable_value(
        ...
        match self.iterable_values.entry(name.into()) {
            ...
            Occupied(entry) => {
                Err(UncatchableError::MultipleIterableValues(entry.key().clone()).into())
            }
        }
    }
