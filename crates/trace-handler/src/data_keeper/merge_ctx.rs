...
impl MergeCtx {
    ...
    pub(crate) fn try_get_generation(&self, position: u32) -> KeeperResult<u32> {
        ...
        let state = self.slider.state_at_position(position).ok_or_else(|| {
            KeeperError::NoElementAtPosition {
                position,
        ...
        match state {
            ExecutedState::Call(CallResult::Executed(Value::Stream { generation, .. })) => {
                Ok(*generation)
            }
            ...
            state => Err(KeeperError::NoStreamState {
                state: state.clone(),
            }),
        }
    }
